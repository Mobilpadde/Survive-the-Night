...
//		sprite.onload = function(){
			var canvas 		= $("#board")[0];
				...
				medicKits	= {},
				imDead		= false,
				texture		= {x: 14, y: ($("select").val()*14)};

			$("select").change(function(){
				texture	= {x: 14, y: $(this).val()*14};
			})
			...
			function movePlayer(){
				...
				if(player.hp <= 0){
					ctx.drawImage(sprite, 0, 28, 14, 14, (player.x - size), (player.y - size), 14, 14);
				}else{
			...
			function handleKits(){
				for(var key in medicKits){
					var k = medicKits[key];
					ctx.drawImage(sprite, 28, 14, 14, 14, (k.x - size), (k.y - size), 14, 14);
