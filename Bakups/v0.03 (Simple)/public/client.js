$(function(){
	...
	function movePlayer(){
		if(player.hp > 0){
			...
			if(movement.up 		&& player.y >= 058){ player.y = player.y - speed; socket.emit("playerMovement", player); };
		...
		ctx.fillStyle = "rgba("+player.color.r+", "+player.color.b+", "+player.color.b+", "+player.hp+")";
		ctx.beginPath();
	...
	function handleClients(){
		...
		for(var key in clients){
			...
			ctx.fillStyle = "rgba("+c.color.r+", "+c.color.b+", "+c.color.b+", "+c.hp+")";
			ctx.beginPath();
